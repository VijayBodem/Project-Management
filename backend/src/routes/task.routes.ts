...
import { createTaskSchema, updateTaskSchema, assignTaskSchema } from "../utils/validation";
...
// Create task
router.post("/", authenticate, validate(createTaskSchema), asyncHandler(async (req: Request, res: Response) => {
  ...
  const member = project.members.find((m: any) => m.user.toString() === req.user!.userId);
...
router.get("/project/:projectId", authenticate, checkProjectMembership, requirePermission(ProjectPermission.VIEW_PROJECT), asyncHandler(async (req: Request, res: Response) => {
  const { assignedTo, status, unassigned, page = "1", limit = "20", sortBy = "createdAt", sortOrder = "desc" } = req.query;
...
router.get("/my-tasks", authenticate, asyncHandler(async (req: Request, res: Response) => {
...
router.patch("/:taskId/assign", authenticate, validate(assignTaskSchema), async (req, res) => {
  ...
  if (newAssignees.length > 0) {
    const projectMemberIds = project.members.map((m: any) => m.user.toString());
    const invalidAssignees = newAssignees.filter((id: string) => !projectMemberIds.includes(id));
  ...
  const previousAssignees = (task.assignedTo || []).map((id: any) => id.toString());
  ...
  const addedAssignees = newAssignees.filter((id: string) => !previousAssignees.includes(id));
  const removedAssignees = previousAssignees.filter((id: string) => !newAssignees.includes(id));
...
router.patch("/:taskId/status", authenticate, asyncHandler(async (req: Request, res: Response) => {
  ...
  if (!hasPermission(userRole, ProjectPermission.CHANGE_TASK_STATUS)) {
    throw new AppError("You don't have permission to change task status", 403);
  }
  ...
  if (oldStatus !== newStatus) {
    ...
    const newPosition = position !== undefined ? position : tasksInNewColumn.length;
  ...
  if (updatedTask && updatedTask.assignedTo && Array.isArray(updatedTask.assignedTo)) {
...
// Update task details
router.patch("/:taskId", authenticate, validate(updateTaskSchema), asyncHandler(async (req: Request, res: Response) => {
  const { taskId } = req.params;
...
router.delete("/:taskId", authenticate, asyncHandler(async (req: Request, res: Response) => {
  ...
  if (!hasPermission(userRole, ProjectPermission.DELETE_TASK)) {
    ...
    if (task.createdBy.toString() !== req.user!.userId && !isOwner) {
      throw new AppError("Only task creator or project owner can delete", 403);
    }
  }
