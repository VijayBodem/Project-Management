...
interface UseTaskRealtimeOptions {
  ...
  onTaskAssigned?: (data: { task: Task; addedAssignees: string[]; removedAssignees: string[] }) => void;
...
export const useTaskRealtime = (options: UseTaskRealtimeOptions = {}) => {
  ...
  const handleTaskCreated = useCallback((data: { task: Task; createdBy: string }) => {
    ...
    if (userId) {
      const isAssigned = data.task.assignedTo?.some(assignee => assignee._id === userId);
      if (!isAssigned) {
        return;
      }
    }
  ...
  const handleTaskUpdated = useCallback((data: { task: Task; updatedBy: string; field: string }) => {
  ...
  const handleTaskAssigned = useCallback((data: { 
    ...
    if (userId) {
      ...
      if (wasAdded) {
        ...
        const isStillAssigned = data.task.assignedTo?.some(assignee => assignee._id === userId);
  ...
  }, [onTaskCreated, onTaskUpdated, onTaskDeleted, onTaskAssigned, projectId, userId]);

  const handleTaskDeleted = useCallback((data: { taskId: string; deletedBy: string }) => {
  ...
  }, [handleTaskCreated, handleTaskUpdated, handleTaskAssigned, handleTaskDeleted]);
};
